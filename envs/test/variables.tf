# =====================================================================
# 🌐 TEST ENVIRONMENT — INPUT VARIABLES
# ---------------------------------------------------------------------
# Defines all input variables for the "test" environment.
...
variable "namespaces" {
  type        = list(string)
  description = "Namespaces to create and apply namespace roles to."
}

# ------------------------------------------------------------
# 👥 USER ACCESS CONTROL (Cluster and Namespace roles)
# ------------------------------------------------------------

# Cluster-wide viewer users
variable "cluster_viewer_users" {
  description = "List of usernames with cluster-wide read-only (viewer) access."
  type        = list(string)
  default     = []
}

# Cluster-wide admin users
variable "cluster_admin_users" {
  description = "List of usernames with cluster-wide admin access."
  type        = list(string)
  default     = []
}

# Namespace-scoped viewer users
# Accepts either:
#   - a flat list (applied to all namespaces)
#   - or a map(namespace → list of usernames)
variable "namespace_viewer_users" {
  description = "Either a map(namespace → list of usernames) or a flat list applied to all namespaces."
  type        = any
  default     = []
}

# Namespace-scoped admin users
# Accepts either:
#   - a flat list (applied to all namespaces)
#   - or a map(namespace → list of usernames)
